...
import { getUpstreamRoot, getActiveSources, EMBEDDING_PROVIDER } from '../lib/config.js';
import { chunkFile } from '../lib/chunker.js';
import { embedAll } from '../lib/embedder.js';
import { extractReleaseMetadata, formatChangelogMarkdown } from '../lib/release-parser.js';
import {
  openDb, closeDb,
 ...
 * @param {{ force: boolean, release: string, extraSources: string }} opts
 */
export async function handler(opts) {
  ...
  console.log('Hindsight Knowledge Base Indexer');
...
function shouldSkipDirectory(name) {
  return name === 'node_modules' || name === '.git' || name === '__pycache__'
    || name === '.next' || name === 'dist' || name === 'venv';
}
