...
export function buildSystemdFiles({ interval, envFile, upstreamDir, dataDir }) {
  ...
  let serviceContent = `[Unit]\nDescription=Hindsight KB Sync\n\n[Service]\nType=oneshot\nExecStart=hindsight-kb sync --upstream-dir ${upstreamDir} --data-dir ${dataDir}\n`;
  if (envFile) {
    serviceContent += `EnvironmentFile=${resolve(envFile)}\n`;
  }

  const timerContent = `[Unit]\nDescription=Hindsight KB Sync Timer\n\n[Timer]\nOnBootSec=60\nOnUnitActiveSec=${seconds}\nPersistent=true\n\n[Install]\nWantedBy=timers.target\n`;
...
function generateSystemd({ interval, envFile, upstreamDir, dataDir }) {
  ...
  const servicePath = join(dir, 'hindsight-kb-sync.service');
  const timerPath = join(dir, 'hindsight-kb-sync.timer');
  ...
  console.log('  systemctl --user enable hindsight-kb-sync.timer');
  console.log('  systemctl --user start hindsight-kb-sync.timer');
}
...
<dict>
    <key>Label</key>
    <string>com.hindsight.kb-sync</string>
    <key>ProgramArguments</key>
    <array>
        <string>hindsight-kb</string>
        <string>sync</string>
...
`;
  ...
  const plistPath = join(dir, 'com.hindsight.kb-sync.plist');
