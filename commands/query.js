...
export function register(program) {
  program
    ...
    .option('--ui', 'Filter to control-plane UI source')
    .option('--go', 'Filter to Go client source')
...
export async function handler(opts) {
  const {
    query, docs, code, tests, integrations, ui, go, releases,
    verify, json, top = '8', offline = false,
  } = opts;

  if (!query || !query.trim()) {
    console.error('Usage: hindsight-kb query <text>');
  ...
  try {
    ...
    if (integrations) sourceFilter = 'integrations';
    else if (ui) sourceFilter = 'control-plane';
    else if (go) sourceFilter = 'go-client';
    else if (releases) sourceFilter = 'releases';

    if (docs) contentTypeFilter = 'docs';
    else if (code) contentTypeFilter = 'code';
    else if (tests) contentTypeFilter = 'test';
