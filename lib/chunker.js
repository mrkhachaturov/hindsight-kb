 ...
 * @param {string} source  - Source category (docs, engine, go-client, etc.)
...
function deriveMetadata(relPath, source) {
  ...
  const langMap = { 'py': 'python', 'go': 'go', 'ts': 'typescript', 'tsx': 'typescript', 'md': 'markdown' };
  ...
  if (ext === 'md') {
    ...
    contentType = source === 'tests' ? 'test' : 'code';
  } else if (ext === 'go' || ext === 'ts' || ext === 'tsx') {
    contentType = 'code';
  }
...
export function chunkFile(content, relPath, source) {
  ...
  const isCodeFile = ['python', 'go', 'typescript'].includes(metadata.language);
