...
function initSchema() {
  ...
  // Migrate: Add release_date column to releases if it doesn't exist
  try {
    db.exec('ALTER TABLE releases ADD COLUMN release_date TEXT');
 ...
 * @param {string|null} indexedRelease - Release tag that indexed these chunks (e.g., 'v0.4.20')
 */
 ...
 * @param {string} sinceRelease - Minimum release tag (e.g., 'v0.4.20')
...
export function getChunksSinceRelease(sinceRelease, limit = 100) {
  ...
  let rows;
  if (release) {
    const sortDate = release.release_date || release.date;
    rows = prepare(`
      ...
      WHERE COALESCE(r.release_date, r.date) >= ?
      ORDER BY COALESCE(r.release_date, r.date) DESC, c.path ASC
      LIMIT ?
    `).all(sortDate, limit);
  } else {
    rows = prepare(`
      SELECT id, path, source, content_type, language, category, indexed_release,
             start_line, end_line, text FROM chunks WHERE indexed_release = ?
      ORDER BY path ASC LIMIT ?
    `).all(sinceRelease, limit);
  }
  return rows.map(r => ({
    id: r.id, path: r.path, source: r.source,
    contentType: r.content_type, language: r.language, category: r.category,
    indexedRelease: r.indexed_release, startLine: r.start_line, endLine: r.end_line, text: r.text,
  }));
}
...
export function insertRelease(metadata) {
  prepare(`
    INSERT OR REPLACE INTO releases (
      tag, date, release_date, commit_hash, previous_tag, commits_count,
      files_changed, kb_files_changed, kb_impact, changelog_json, indexed_at
    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).run(
    metadata.tag, metadata.date, metadata.release_date || metadata.date,
    metadata.commit_hash || metadata.commit, metadata.previous_tag || null,
    metadata.commits_count || 0, metadata.files_changed || 0,
    metadata.kb_files_changed || 0, metadata.kb_impact || 'unknown',
    metadata.changelog ? JSON.stringify(metadata.changelog) : null, Date.now()
  );
}
...
export function getLatestRelease() {
  return prepare('SELECT * FROM releases ORDER BY COALESCE(release_date, date) DESC LIMIT 1').get() || null;
}
...
export function getCurrentIndexedRelease() {
  const row = prepare(`
    ...
    ORDER BY COUNT(*) DESC, COALESCE(r.release_date, r.date, c.indexed_release) DESC
    LIMIT 1
  `).get();
...
export function getReleaseHistory(limit = 10) {
  return prepare('SELECT * FROM releases ORDER BY COALESCE(release_date, date) DESC LIMIT ?').all(limit);
}

/**
 * Get release by tag.
 * @param {string} tag - Release tag (e.g., 'v0.4.20')
